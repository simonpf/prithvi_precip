...
def load_and_interp_climatology(time: np.datetime64, data_dir: Path) -> np.ndarray:
    ...
    with xr.open_dataset(sfc_file_l, engine="h5netcdf", chunks=None, cache=False) as sfc_data:
        for var in SURFACE_VARS:
            data_sfc_l.append(sfc_data[var].data.copy().astype(np.float32))
    del sfc_data
    data_sfc_l = np.stack(data_sfc_l)
    data_sfc_r = []
    with xr.open_dataset(sfc_file_r, engine="h5netcdf", chunks=None, cache=False) as sfc_data:
        for var in SURFACE_VARS:
            data_sfc_r.append(sfc_data[var].data.copy().astype(np.float32))
    del sfc_data
    ...
    data_vert_l = []
    with xr.open_dataset(vert_file_l, engine="h5netcdf", chunks=None, cache=False) as data_vert:
        for var in VERTICAL_VARS:
            data_vert_l.append(np.flip(data_vert[var].data.copy().astype(np.float32), 0))
    del data_vert
    ...
    data_vert_r = []
    with xr.open_dataset(vert_file_r, engine="h5netcdf", chunks=None, cache=False) as data_vert:
        for var in VERTICAL_VARS:
            data_vert_r.append(np.flip(data_vert[var].data.copy().astype(np.float32), 0))
    del data_vert
...
def load_dynamic_input(path: Path, slcs: Optional[Dict[str, slice]] = None) -> torch.Tensor:
    ...
    with xr.open_dataset(path, engine="h5netcdf", chunks=None, cache=False) as data:
        for var in SURFACE_VARS:
            all_data.append(data[var].__getitem__(slcs).data.copy()[None].astype(np.float32))
        for var in VERTICAL_VARS:
            all_data.append(data[var].__getitem__(slcs).copy().astype(np.float32))
    del data
