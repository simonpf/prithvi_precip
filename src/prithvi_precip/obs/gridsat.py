"""
prithvi_precip.obs.gridsat
==========================

This module provides functionality to extract satellite observations from the
 GridSat dataset.
"""
...
CHANNELS = ["vschn", "irwvp", "irwin_cdr"]
...
def extract_observations(
    ...
    Args:
e       base_path: Path object pointing to the directory to which to write
    ...
    with xr.open_dataset(goes_file) as data:
        ...
        for ch_ind, ch_name in enumerate(CHANNELS):

            ch_ind = ch_ind + 1

            obs = data[[ch_name]][{"time": 0}].rename({ch_name: "observations"})
            if ch_ind == 1:
            ...
            obs_name = f"gridsat_b1_ch{ch_ind}"
...
def extract_observations_day(
    ...
    for step in time_steps:
        recs += gridsat_b1.get(TimeRange(step, step + np.timedelta64(30, "m")))
    for rec in recs:
        try:
