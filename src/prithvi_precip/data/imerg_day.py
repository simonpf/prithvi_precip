"""
prithvi_precip.data.imerg_day
=============================

Functionality to extract training data from the IMERG Final daily product. This is more suitable
for create reference data for forecast of daily to weekly accumulations.
"""
...
from pansat.products.satellite.gpm import (
    l3b_day_3imerg_ms_mrg_v07b,
    l3b_daye_3imerg_ms_mrg_v07b
)
...
def extract_imerg_precip(
    ...
    if len(recs) == 0:
        recs = l3b_daye_3imerg_ms_mrg_v07b.get(time_range)
    ...
    for time in np.arange(start_time, end_time, np.timedelta64(granularity, "D")):
        data_t = data.interp(time=time.astype("datetime64[ns]"), method="nearest")
        time = to_datetime(data_t["time"].data)
        fname = time.strftime(f"imerg_day_{accumulate}/{time.year:04}/{time.month:02}/imerg_day_%Y%m%d%H%M.nc")
